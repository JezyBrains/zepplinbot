#!/usr/bin/env python3
"""Quick script to identify new crashes from pasted data."""
...
print(f"\nTotal crashes in paste: {len(crashes)}")
...
if found_idx >= 0:
    ...
    print(f"\n✅ Found last crash (2.95x) at position {found_idx}")
    print(f"\n🆕 NEW crashes (newest to oldest):")
    for i, crash in enumerate(new_crashes):
        print(f"   {i+1}. {crash}x")
    print(f"\n📊 Total NEW crashes: {len(new_crashes)}")
else:
    print(f"\n⚠️  Last crash (2.95x) not found in pasted data")
    print(f"   All {len(crashes)} crashes might be new")
